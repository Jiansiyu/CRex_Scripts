...
TString prepcut;
TString generalcut;
TString generalcutR="R.tr.n==1 && R.vdc.u1.nclust==1&& R.vdc.v1.nclust==1 && R.vdc.u2.nclust==1 && R.vdc.v2.nclust==1 &&  fEvtHdr.fEvtType==1 && R.gold.p > 0.94 && R.gold.p < 0.98  ";
...
Int_t OpticsGraphicCutPro(UInt_t runID,TString folder="/home/newdriver/Storage/Research/PRex_Experiment/PRex_Replay/replay/Result") {
	// prepare the data
...
//		delete SieveRecCanvas->GetPrimitive("Projection");
	...
	chain->Project(selectedSievehh->GetName(),
			...
			Form("sqrt((%s.gold.th-%f)^2+ (%s.gold.ph-%f)^2)<0.0035 && %s",
	...
	if(HRS=="L"){
		...
		TH1F *HallR_NMR = new TH1F("HallR_NMR", "HallR_NMR", 1000, 0.3, 0.9);
	...
	// plot the dp and fit
	TH1F *momentum=new TH1F(Form("C-12 gold.p"),"C-12 gold.p",800,0.94,0.98);
	...
	momentum->Fit("gaus","RQ0","ep",CGroundp-0.001,CGroundp+0.0005);

	double_t fgroundCrystalballPar[5];
	TF1 *fgroundCrystalball = new TF1("fgroundCrystal", "crystalball",
			momentum->GetFunction("gaus")->GetParameter(1)
						- 5 * momentum->GetFunction("gaus")->GetParameter(2),
						momentum->GetFunction("gaus")->GetParameter(1)
						+ 4 * momentum->GetFunction("gaus")->GetParameter(2));
		fgroundCrystalball->SetParameters(
				momentum->GetFunction("gaus")->GetParameter(0),
				momentum->GetFunction("gaus")->GetParameter(1),
				momentum->GetFunction("gaus")->GetParameter(2), 1.64, 1.1615);

	momentum->Fit("fgroundCrystal","RQ","ep",fgroundCrystalball->GetXmin(),fgroundCrystalball->GetXmax());
	fgroundCrystalball->GetParameters(fgroundCrystalballPar);
	fgroundCrystalball->Draw("same");

	double_t ffirstGuasPar[3];
	TH1F *test=(TH1F *)momentum->Clone("fitTest");
	test->GetXaxis()->SetRangeUser(momentum->GetXaxis()->GetXmin(),C1stp+0.0005);
	C1stp=test->GetXaxis()->GetBinCenter(test->GetMaximumBin());

	TF1 *ffirstGuas=new TF1 ("firststatesgaus","gaus",C1stp-2*fgroundCrystalballPar[2],C1stp+1*fgroundCrystalballPar[2]);
	momentum->Fit("firststatesgaus","R0Q","ep",ffirstGuas->GetXmin(),ffirstGuas->GetXmax());
	ffirstGuas->GetParameters(ffirstGuasPar);
//	ffirstGuas->Draw("same");
	...
	std::copy(ffirstCrystalPar,ffirstCrystalPar+5,fCrystalMomentumPar+5);
	fCrystalMomentum->SetParameters(fCrystalMomentumPar);
	momentum->Fit("fCrystalMomentum","RQ0","ep",fCrystalMomentum->GetXmin(),fCrystalMomentum->GetXmax());
	fCrystalMomentum->GetParameters(fCrystalMomentumPar);
	fCrystalMomentum->Draw("same");
	// plot the information on canvas



	{
		double fiterror=TMath::Sqrt(fgroundCrystalball->GetParError(1)*fgroundCrystalball->GetParError(1)+ffirstCrystal->GetParError(1)*ffirstCrystal->GetParError(1));
		TLatex *text1=new TLatex(fCrystalMomentumPar[1],fCrystalMomentumPar[0],Form("P_0=%1.4fGeV#pm %1.3fMeV",fgroundCrystalballPar[1],1000*fgroundCrystalball->GetParError(1)));
		TLatex *text2=new TLatex(fCrystalMomentumPar[6],fCrystalMomentumPar[5],Form("P_0=%1.4fGeV#pm %1.3fMeV",ffirstCrystalPar[1],1000.0*ffirstCrystal->GetParError(1)));
		TLatex *text3 = new TLatex(
				fCrystalMomentumPar[1] * 0.5 + fCrystalMomentumPar[6] * 0.5,
				fCrystalMomentumPar[0] * 0.5 + fCrystalMomentumPar[5] * 0.5,
				Form("#DeltaP=%1.4f#pm%1.3f MeV", 1000.0*(fgroundCrystalballPar[1]-ffirstCrystalPar[1]),1000*fiterror));

		text1->Draw("same");
		text2->Draw("same");
		text3->Draw("same");

//		double fiterror=TMath::Sqrt(fCrystalMomentum->GetParError(1)*fCrystalMomentum->GetParError(1)+fCrystalMomentum->GetParError(6)*fCrystalMomentum->GetParError(6));
//		TLatex *text1=new TLatex(fCrystalMomentumPar[1],fCrystalMomentumPar[0],Form("P_0=%1.4fGeV#pm %1.3fMeV",fCrystalMomentumPar[1],1000*fCrystalMomentum->GetParError(1)));
//		TLatex *text2=new TLatex(fCrystalMomentumPar[6],fCrystalMomentumPar[5],Form("P_0=%1.4fGeV#pm %1.3fMeV",fCrystalMomentumPar[6],1000*fCrystalMomentum->GetParError(6)));
//		TLatex *text3 = new TLatex(
//				fCrystalMomentumPar[1] * 0.5 + fCrystalMomentumPar[6] * 0.5,
//				fCrystalMomentumPar[0] * 0.5 + fCrystalMomentumPar[5] * 0.5,
//				Form("#DeltaP=%1.4f#pm%1.3f MeV", 1000.0*(fCrystalMomentumPar[1]-fCrystalMomentumPar[6]),1000*fiterror));
//
//		text1->Draw("same");
//		text2->Draw("same");
//		text3->Draw("same");
	...
	TH2F *focalThetaPhi=new TH2F("focalthetaPhi","focalthetaPhi",500,-0.05,0.05,500,-0.05,0.05);
	chain->Project(focalThetaPhi->GetName(),Form("%s.tr.r_th:%s.tr.r_ph",HRS.Data(),HRS.Data()),Form("%s && %s",generalcut.Data(),cutg->GetName()));
//	focalThetaPhi->SetContour(10);
	focalThetaPhi->Draw("CONT LIST");


	SieveRecCanvas->cd(2)->cd(4);
	TH2F *focalxy=new TH2F("focalxy","focalxy",500,-0.2,0.2,500,-0.2,0.2);
	chain->Project(focalxy->GetName(),Form("%s.tr.r_x:%s.tr.r_y",HRS.Data(),HRS.Data()),Form("%s && %s",generalcut.Data(),cutg->GetName()));
	focalxy->SetContour(10);
	focalxy->Draw("CONT LIST");
...
/*	double_t fgroudGausPar[3];
...

	{
		...
		TH1F *firstexcited_temp=new TH1F("C-12 gold.p_temp","C-12 gold.p_temp",2000,0.94,0.98);
