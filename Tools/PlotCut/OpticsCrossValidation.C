...
//	mainPatternCanvas->AddExec("ex", "DynamicCanvasSubtree()");
...
//Used for Plot Trees after cut
//After the Dynamic Canvas Plot, select the central sieve, and save as new Tree
// In this Function, it randomly select Half of the event, and calculate the Momentum
void PlotMomAll(TString filename="/home/newdriver/Storage/Research/CRex_Experiment/RasterReplay/Replay/Result"){

}

void DynamicCanvasSubtree(){
...
//		delete SieveRecCanvas->GetPrimitive("Projection");
	...
	// select the tree
	TFile subTreeFile(Form("./temp/prexSubTree_%d.root",globalFileRunID),"recreate");
	std::cout<<"\n Total Entries:"<<(chain->GetTree()->GetEntries())<<std::endl;
	std::cout<<"\n Total Entries After cut:"<<(chain->GetTree()->CopyTree(Form("%s && %s",generalcut.Data(),cutg->GetName()))->GetEntries())<<std::endl;

	TTree *subTree=chain->GetTree()->CopyTree("");//Form("%s && %s",generalcut.Data(),cutg->GetName()));
	subTree->Write();
	subTreeFile.Write();
	subTreeFile.Close();
...
//		delete SieveRecCanvas->GetPrimitive("Projection");
	...
	std::cout<<"\n\n\n\nMomentum Entries:::"<<momentum->GetEntries()<<"\n\n\n\n"<<std::endl;
...
//		PeakMomInfor->SetTextAngle(30);
	...
	for(auto Spectro_index=SpectrumFitResult.size()-1;Spectro_index>=1;Spectro_index--){
		...
		TLatex *deltaEtxt=new TLatex(SpectrumFitResult.at(Spectro_index)->GetMean(), SpectrumFitResult.at(Spectro_index)->GetHeight()/10,Form("#DeltaP=%1.3f #pm %1.2fMeV",SeparationE*1000.0, SeparationError*1000.0));
