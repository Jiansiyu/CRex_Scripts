 ...
 *      Author: newdriver
 *      Used for Check the Rasted and UnRasted run RMS value
 */
...
double getCentralP(TChain *chain,TString HRS){
...
// carbon target
// load the cut file
int OpticsGraphicCutDpCheck(UInt_t runID,UInt_t runID1,
                            TString cutFile =
                            "/home/newdriver/Storage/Research/PRex_Workspace/PREX-MPDGEM/PRexScripts/Tools/PlotCut/Result/Cut20200915/WithOutMomCut",
                            TString folder =
...
// check the raster affect on the sieve
int RasterCheck(UInt_t runID,UInt_t runID1,
                            TString cutFile =
                            "/home/newdriver/Storage/Research/PRex_Workspace/PREX-MPDGEM/PRexScripts/Tools/PlotCut/Result/Cut20200915/WithOutMomCut",
                TString cutFile1 =
                "/home/newdriver/Storage/Research/PRex_Workspace/PREX-MPDGEM/PRexScripts/Tools/PlotCut/Result/Cut20200915/WithOutMomCut",
    ...
    auto chain0=LoadrootFile(runID,folder);
    auto chain1=LoadrootFile(runID1, folder);
    ...
    std::map<int,std::map<int, TCutG *>>SieveCut0;

    // load the cut informations
    for (int16_t col = 0; col < NSieveCol; col++) {
        for (int16_t row = 0; row < NSieveRow; row++) {
            auto cutg = (TCutG *) gROOT->FindObject(
                    Form("hcut_R_%d_%d_%d", FoilID, col, row));
            if (cutg) {
                cutg->SetName(Form("hcut_R_%d_%d_%d_0", FoilID, col, row));
                SieveCut0[col][row]=cutg;
            }
        }
    }
    cutFileIO->Close();
    ...
    // load the cut file
    if(!cutFile1.EndsWith(".root")){
        cutFile1=Form("%s/prexRHRS_%d_-1.root.FullCut.root",cutFile1.Data(),runID1);
    }
    TFile *cutFileIO1=new TFile(cutFile1.Data(),"READ");
    if(cutFileIO1->IsZombie()){
        std::cout<<"[ERROR]:: CAN NOT FIND CUT FILE \" "<<cutFile1.Data()<<"\""<<std::endl;
        return -1;
    }

    std::map<int,std::map<int, TCutG *>>SieveCut1;
    ...
    for (int16_t col = 0; col < NSieveCol; col++) {
        for (int16_t row = 0; row < NSieveRow; row++) {
            auto cutg = (TCutG *) gROOT->FindObject(
                    Form("hcut_R_%d_%d_%d", FoilID, col, row));
            if (cutg) {
                cutg->SetName(Form("hcut_R_%d_%d_%d_1", FoilID, col, row));
                SieveCut1[col][row]=cutg;
    ...
    mainPatternCanvas->Divide(1,2);
    mainPatternCanvas->cd(1)->Divide(2,1);
    mainPatternCanvas->cd(1)->cd(1);
    TH2F *TargetThPhHH0=(TH2F *)gROOT->FindObject("th_vs_ph0");
    if(TargetThPhHH0) TargetThPhHH0->Delete();
    TargetThPhHH0=new TH2F("th_vs_ph0","th_vs_ph0",1000,-0.03,0.03,1000,-0.045,0.05);
    chain0->Project(TargetThPhHH0->GetName(),Form("%s.gold.th:%s.gold.ph",HRS.Data(),HRS.Data()),generalcut.Data());
    TargetThPhHH0->Draw("zcol");
    for (auto colIter=SieveCut0.begin();colIter!=SieveCut0.end();colIter++){
        for (auto rowIter = (colIter->second).begin(); rowIter!=(colIter->second).end();rowIter++){
            rowIter->second->Draw("same");
            TLatex *text=new TLatex(rowIter->second->GetMean(1),rowIter->second->GetMean(2),Form("(%d,%d)",colIter->first,rowIter->first));
            text->Draw("same");
        }
    }

    mainPatternCanvas->cd(1)->cd(2);
    TH2F *TargetThPhHH1=(TH2F *)gROOT->FindObject("th_vs_ph1");
    if(TargetThPhHH1) TargetThPhHH1->Delete();
    TargetThPhHH1=new TH2F("th_vs_ph1","th_vs_ph1",1000,-0.03,0.03,1000,-0.045,0.05);
    chain1->Project(TargetThPhHH1->GetName(),Form("%s.gold.th:%s.gold.ph",HRS.Data(),HRS.Data()),generalcut.Data());
    TargetThPhHH1->Draw("zcol");
    for (auto colIter=SieveCut1.begin();colIter!=SieveCut1.end();colIter++){
        for (auto rowIter = (colIter->second).begin(); rowIter!=(colIter->second).end();rowIter++){
            rowIter->second->Draw("same");
            TLatex *text=new TLatex(rowIter->second->GetMean(1),rowIter->second->GetMean(2),Form("(%d,%d)",colIter->first,rowIter->first));
            text->Draw("same");
    ...
    //loop on the sieve holes and get the momentum functions
    for (auto colIter=SieveCut0.begin();colIter!=SieveCut0.end();colIter++){
        for (auto rowIter = (colIter->second).begin(); rowIter!=(colIter->second).end();rowIter++){
            auto colID=colIter->first;
            auto rowID=rowIter->first;
            // check in the second file, it have this cut
            if ((SieveCut1.find(colID)!=SieveCut1.end()) &&(SieveCut1[colID].find(rowID)!=SieveCut1[colID].end())){
                // if both of the run have this cut, try to calculate this cut
