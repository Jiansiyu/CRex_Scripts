...
//	mainPatternCanvas->AddExec("ex", "DynamicCoordinates()");
    mainPatternCanvas->AddExec("ex", "DynamicCanvasTest()");
...
double getCentralP(TChain *chain){
    TH1F *eventIDhh=new TH1F("eventID","eventID",800000,0,800000);
    chain->Project(eventIDhh->GetName(),"fEvtHdr.fRun");
    int eventID=int(eventIDhh->GetMean());
    TString HRS("L");
    if (eventID > 20000) HRS="R";
...
//	auto C1stp=2.1565;//CGroundp-0.016504;
    ...
    momentum->Write(Form("Momentum%dHist",eventID));
...
void ComparePlot(){
    TCanvas *canv=new TCanvas("Canv","Canv",1960,1600);
    canv->Draw();
    canv->cd();
    canv->SetLogy();

    TLegend *legend=new TLegend(0.3,0.3);


    TFile *f1=TFile::Open("test_temp_1.root");
    TH1F *hist1=(TH1F *)f1->Get("Momentum2672Hist");
    legend->AddEntry(hist1,hist1->GetName());
    auto fun1=hist1->GetFunction("fCrystalMomentum");
    hist1->Draw();
    TLatex *textg1=new TLatex(fun1->GetParameter(1),fun1->GetParameter(0),Form("%f",fun1->GetParameter(1)));
    textg1->Draw("same");
    TLatex *textf1=new TLatex(fun1->GetParameter(6),fun1->GetParameter(5),Form("%f",fun1->GetParameter(6)));
    textf1->Draw("same");

    TFile *f2=TFile::Open("test_temp_2.root");
    TH1F *hist2=(TH1F *)f2->Get("Momentum21737Hist");
    hist2->SetLineColor(6);
    legend->AddEntry(hist2,hist2->GetName());
    hist2->Draw("same");

    auto fun2=hist2->GetFunction("fCrystalMomentum");
    TLatex *textg2=new TLatex(fun2->GetParameter(1),fun2->GetParameter(0),Form("%f",fun2->GetParameter(1)));
    textg2->Draw("same");
    TLatex *textf2=new TLatex(fun2->GetParameter(6),fun2->GetParameter(5),Form("%f",fun2->GetParameter(6)));
    textf2->Draw("same");
    textg2->SetTextColor(6);
    textf2->SetTextColor(6);
