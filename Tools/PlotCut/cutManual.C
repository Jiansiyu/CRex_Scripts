...
void SavePatternHole(double momentumSigmaCut=3.0){
	...
	for (int row_iter = row_min; row_iter<row_min+row_count;row_iter++){
		...
		if(cutg){
			...
			std::cout<<"Maximum Peak :: "<<(sieveholemomentum[row_iter-row_min]->GetXaxis()->GetBinCenter(sieveholemomentum[row_iter-row_min]->GetMaximumBin()))<<std::endl;
...
//			SaveCheckCanvas->cd(2)->cd(row_iter - row_min + 1)->SetLogy();
			...
			sieveholemomentum[row_iter-row_min]=new TH1F(Form("hcut_R_%d_%d_%d_h_momentum", FoilID, col, row_iter),Form("hcut_R_%d_%d_%d_momentum", FoilID, col, row_iter),800,0.94,0.96);
			chain->Project(sieveholemomentum[row_iter-row_min]->GetName(),Form("%s.gold.p",HRS.Data()),Form("%s && %s",cutg->GetName(),generalcut.Data()));
			sieveholemomentumGausFit[row_iter-row_min]=new TF1(Form("1ststatesDpgaushcut_R_%d_%d_%d", FoilID, col, row_iter),"gaus",
...
//			SavePatternHole_P1();
	...
	if (event==kButton1Down) {
		...
		if(SieveRecCanvas){
			...
			TH1F *sieveholemomentum=new TH1F(Form("hcut_R_%d_%d_%d_h_momentum_check", FoilID, col, row),Form("hcut_R_%d_%d_%d_momentum_check", FoilID, col, row),600,0.94,0.96);
