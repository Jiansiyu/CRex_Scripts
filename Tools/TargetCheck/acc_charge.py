...
class parityDatabase(object):
    ...
    def get_info_all(self):
        ...
        # argv
        parser = argparse.ArgumentParser(description="", usage=self.get_usage())
        ...
        if args.run is None and args.list is None:
            ...
            print
            "For all production runs ", runs[0], runs[-1]
        ...
        for run in result:
            ...
            if target_type is None or '48Ca' not in target_type:
                if run.get_condition_value("slug") < 3999:
                    print
                    "Non-production target run"
                    print
                    run.get_condition_value("target_type")
                    pass_cut = False
            ...
            if pass_cut:
                ...
                if not summary_output:
                    ...
                    print
                    "=== Prompt summary output does not exist for run: ", runno, ", skip this run for Good charge"
                    charge2 = "0"
                    prompt_time = "0"
                    ...
                    if 'nan' in summary_output[3]:
                        print
                        "=== No good event processed for run :", runno, " , skip this run"
                        prompt_time = "0"
                        charge2 = "0"
                    else:
                ...
                if length is None:
                    print
                    "=== Run did not end properly...", runno, ", let's skip this"
                else:
